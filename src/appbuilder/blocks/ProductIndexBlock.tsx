...
class ProductIndexBlock extends Component<ProductIndexProps, ProductIndexState> {
  ...
  constructor(props: ProductIndexProps) {
    ...
    this.state = {
      isLoading: false,
      isMultiColumn: props.isMultiColumn === undefined || props.isMultiColumn
    };
  }
  ...
  renderTitle = (item: CommerceTypes.Product) => (): JSX.Element => {
    return (
      <Text
  ...
  renderProductItem = () => {
    return (item: CommerceTypes.Product): JSX.Element => {
      ...
      const { brandStyle, priceStyle } = this.props;
      return (
        <ProductItem
          image={image}
          buttonProps={{ palette }}
          ...
          orientation={this.state.isMultiColumn ? 'vertical' : 'horizontal'}
  ...
  render(): JSX.Element | null {
    ...
    const { keyword, renderNoResult, dataSource, containerStyle } = this.props;
    ...
    const indexProps: any = {
      ...
      listStyle: [PIPStyle.container, containerStyle],
      renderRefineActionBar: this.renderRefineActionBar,
      commerceDataSource: chooseDataSource(dsType, apiConfig),
...
export default ProductIndexBlock;
